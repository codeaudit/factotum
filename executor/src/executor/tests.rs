...
fn worker_manager_spawn_and_exit() {
    ...
    let handle = spawn_worker_manager(tx.clone(), rx, VecDeque::new(), 2, pool.clone(), "/tmp/fake_command".to_string());
    tx.send(Dispatch::StopProcessing);
...
fn new_job_request_success() {
    ...
    new_job_request(tx.clone(), &mut requests_queue, pool.clone(), job_request.clone());
...
fn process_job_request_failure() {
    ...
    process_job_request(tx.clone(), &mut requests_queue, pool.clone(), "/tmp/fake_command".to_string());
...
fn complete_job_request_success() {
    ...
    complete_job_request(tx, job_request);
...
fn failed_job_request_success() {
    ...
    failed_job_request(tx, job_request);
